const animationTimeline = () => {
  ...
  // Safely split text only if elements exist
  if (textBoxChars) {
  ...
  tl.to(".container", 0.1, { visibility: "visible" })
    ...
    // BALLOONS: Changed from fixed 1400px to viewport-based percentages
    .staggerFromTo(
      ".baloons img",
      2.5,
      { opacity: 0.9, y: "120vh" },
    ...
    // DP IMAGE: Reduced scale from 3.5 to 1.5 to prevent overflow on small screens
    .from(
      ".girl-dp",
      0.5,
      { scale: 1.5, opacity: 0, x: 25, y: -25, rotationZ: -45 },
      "-=2",
    )
    ...
    // CONFETTI: Reduced scale from 80 to 30 so it doesn't create huge scrollbars
    .staggerTo(
      ".eight svg",
      1.5,
      {
    ...
    .from(".nine", 0.8, { opacity: 0, y: 30 })
    .from("#mockForm", 0.8, { opacity: 0, y: 20, ease: Power2.easeOut });
...
// Properly wait for the JSON data before starting animations
fetchData().then(() => {
  animationTimeline();
});
